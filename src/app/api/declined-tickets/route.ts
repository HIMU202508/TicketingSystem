...
function mapRowToDeclinedTicket(row: any): DeclinedTicket {
  return {
    id: row.id,
    ...
    declineReason: row.rejection_reason, // Map from old column name
    declinedBy: row.rejected_by, // Map from old column name
    declinedAt: row.rejected_at, // Map from old column name
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use declined_tickets table with old column names
    let query = supabaseAdmin
      .from('declined_tickets')
      .select('id, ticket_id, ticket_number, device_type, owner_name, facility, original_description, rejection_reason, rejected_by, rejected_at, created_at', { count: isExport ? 'exact' : 'estimated' })
      .order('rejected_at', { ascending: false })

    // Apply filters using old column names
    if (search) {
      query = query.or(`ticket_number.ilike.%${search}%,device_type.ilike.%${search}%,owner_name.ilike.%${search}%,rejection_reason.ilike.%${search}%`)
    }
    ...
    if (rejectedBy) {
      query = query.eq('rejected_by', rejectedBy)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'stats') {
      // Get decline statistics using declined_tickets table
      ...
      const { data: todayCount } = await supabaseAdmin
        .from('declined_tickets')
        .select('id', { count: 'exact', head: true })
        .gte('rejected_at', new Date().toISOString().split('T')[0])
      ...
      const { data: declinedByStats } = await supabaseAdmin
        ...
        .then(({ data }) => {
          const stats: Record<string, number> = {}
          data?.forEach(item => {
            const declinedBy = item.rejected_by || 'Unknown'
